...
In the perfect world then one just have to provide the .NET-Type of the custom logger-interface as value for the `loggerType`-parameter. But the .NET JIT compiler can performing inlining so the custom logger-interface might get inlined completely and the `loggerType` becomes a lie. Older versions of NLog (Before NLog ver. 4.5) will trust the `loggerType`, and will perform filtering based on a type that doesn't exist in the StackTrace.
