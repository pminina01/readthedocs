...
The [rules](Configuration-file#rules) redirects logger output to the wanted output targets. The rules also provides [filtering options](Filtering-log-messages#routing) to output only relevant logevents. Usually based on logger-name and loglevel (Ex. `name="*" minlevel="Trace"` means everything).
